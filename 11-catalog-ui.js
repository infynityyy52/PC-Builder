...
function openReplacement(category) {
  ...
  if (isLightMode()) { if (typeof knowledgeLightTrainingActive==='function' && knowledgeLightTrainingActive()) showKnowledgeLightLesson('replacement', null, category); else lightCoachEvent('info','Замена деталей',`Здесь ты выбираешь деталь той же категории вместо установленной. В Лайте удобно пробовать разные варианты: после установки я проверю, совместима ли новая деталь с остальными компонентами.`,7000,true); }
}
function openCatalog() {
  ...
  if (isLightMode()) { if (typeof knowledgeLightTrainingActive==='function' && knowledgeLightTrainingActive()) showKnowledgeLightLesson('catalog'); else lightCoachEvent('info','Каталог комплектующих',`В Лайт-режиме здесь фиксированные базовые цены и все обычные детали доступны постоянно. Никаких рыночных наценок, SOLD OUT или рероллов нет. Выбери деталь, которую хочешь изучить или купить.`,7000,true); }
}
...
function renderCatalog() {
  ...
  wrap.querySelectorAll('[data-cat]').forEach((btn) => btn.addEventListener('click', () => { catalog.category = btn.dataset.cat; catalog.group = null; catalog.series = null; save(); renderCatalog(); if (isLightMode() && typeof knowledgeLightTrainingActive === 'function' && knowledgeLightTrainingActive()) showKnowledgeLightLesson('select-category', null, btn.dataset.cat); }));
