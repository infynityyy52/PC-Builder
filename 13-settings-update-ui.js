...
- Levels 1–3 now continue as hands-on guided courses inside Light Mode instead of ending after a text tutorial.
- Guided training follows the flow: task → spotlight → action → result. The required interface element is highlighted and unrelated clicks are blocked until the task is completed.
- The training now introduces the player's workspace first: My PC, Earnings, the Light Mode money source, and the component catalog are explained through direct interaction.
- Modal windows and overlays that would interfere with the next task are closed automatically. For example, after buying a component, the catalog closes automatically when the next task requires using the earnings panel or clicker.
- Skip is disabled for knowledge levels 1–3 so the beginner courses cannot be accidentally bypassed.
- Added level-specific Light Mode course names, progress tracking, coaching text, animated modal appearance, and animated task transitions.
- Light Mode PC multiplier is now more noticeable: every +5 PC power above the Light Mode starting build gives +0.10× multiplier.
- Synchronized the Light Mode starter 8 GB DDR3 memory with the catalog version: it now uses the same 9-point performance value while remaining a non-purchasable starter component.
- Added the Ryzen 9 9950X3D to the AMD → Ryzen 9 catalog in all modes that use the shared catalog.
- Updated the AMD achievement so its top-tier Ryzen 9 requirement is now specifically the Ryzen 9 9950X3D.

v2.0.3 — PERFECT EXAM BUILDS AND BALANCED SCORING
...
function showLanguagePrompt(){
  ...
  // Предлагаем русский только тем, у кого основной язык браузера — русский.
  // Если браузер работает на английском, popup не должен появляться вообще.
  const browserLanguage=String(navigator.language || '').toLowerCase();
  if(!browserLanguage.startsWith('ru')) return;
...
- Уровни 1–3 после вводного курса продолжают обучение уже внутри Лайт-режима, а не заканчиваются на тексте.
- Основной формат практики: задача → подсветка → действие → результат. Нужный элемент интерфейса подсвечивается, а посторонние клики временно блокируются.
- В начале практики обучение знакомит с рабочим местом игрока: «Мой ПК», «Заработок», источник денег в Лайте и каталог комплектующих.
- Окна и наложения, мешающие следующему заданию, закрываются автоматически. Например, после покупки детали каталог закрывается, когда следующим действием становится получение денег через панель заработка или кликер.
- Для уровней 1–3 отключена кнопка «Пропустить», чтобы новичок не мог случайно перескочить обязательную практику.
- Добавлены отдельные названия курсов Лайт-режима, прогресс, свои подсказки, анимации появления модалок и переходов между заданиями.
- Множитель ПК в Лайт-режиме теперь растёт заметнее: каждые +5 мощности ПК сверх стартовой сборки дают +0.10× к множителю.
- Стартовые 8 ГБ DDR3 синхронизированы с каталоговой версией: теперь стартовая память имеет те же 9 очков производительности, но остаётся отдельной стартовой деталью и не продаётся в каталоге.
- В каталог AMD → Ryzen 9 добавлен Ryzen 9 9950X3D во всех режимах, использующих общий каталог.
- Достижение AMD с требованием топового Ryzen 9 теперь требует именно Ryzen 9 9950X3D.

v2.0.3 — ИДЕАЛЬНЫЕ СБОРКИ И ПЕРЕРАСЧЁТ ЭКЗАМЕНА
...
function openTutorialSkipConfirm(){
  const modal=$('tutorialSkipConfirmModal');
  if(!modal)return;
  modal.classList.remove('hidden');
  modal.setAttribute('aria-hidden','false');
}
function closeTutorialSkipConfirm(){
  const modal=$('tutorialSkipConfirmModal');
  if(modal){modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');}
}
function skipTutorialToMainMenu(){
  closeTutorialSkipConfirm();
  if(firstVisitAutoTimer){clearTimeout(firstVisitAutoTimer);firstVisitAutoTimer=null;}
  ...
  $('tutorialSpotlight')?.style && ($('tutorialSpotlight').style.display='none');
  closeMenuModal('tutorialModal');
  closeFirstVisitPanel();
  if(state?.mode==='light'){state.lightTrainingEnabled=false;save();}
  showMainMenu();
}
...
function renderUpdateLog(){
  const wrap=$('updateLogContent');
  if(!wrap)return;
  const log=getLocalizedUpdateLog();
  wrap.innerHTML='';
  const sections=log.split(/\n\s*\n(?=v\d+\.\d+\.\d+\s+—)/).map(s=>s.trim()).filter(Boolean);
  sections.forEach(section=>{
    const lines=section.split('\n').map(line=>line.trim()).filter(Boolean);
    if(!lines.length)return;
    const card=document.createElement('article');
    card.className='update-log-entry';
    ...
    if(items.children.length)card.appendChild(items);
...
function closeAllMenuModals() {
  ['infoModal', 'settingsModal', 'achievementsModal', 'updateLogModal', 'tutorialModal', 'tutorialSkipConfirmModal'].forEach(closeMenuModal);
...
function openLightTrainingConfirm(){
  const modal=$('lightTrainingConfirmModal');
  const toggle=$('lightTrainingToggle');
  if(!modal)return;
  if(toggle)toggle.checked=false;
  modal.classList.remove('hidden');
  modal.setAttribute('aria-hidden','false');
}
function closeLightTrainingConfirm(){
  const modal=$('lightTrainingConfirmModal');
  if(modal){
    modal.classList.add('hidden');
    modal.setAttribute('aria-hidden','true');
...
function confirmLightTrainingStart(){
  const training=!!$('lightTrainingToggle')?.checked;
  closeLightTrainingConfirm();
  start('light');
  state.lightTrainingEnabled=training;
  lightTrainingActive=training;
  firstVisitTrainingActive=false;
  save();
  render();
  if(training)lightCoachEvent('welcome','Обучение включено.','В этом прохождении Лайт будет давать контекстные подсказки: объяснять детали, причины несовместимости и сильные решения сборки. Экзамен остаётся доступен.',15000,true);
}


/* ===== v2.0.4 KNOWLEDGE-BASED FIRST-VISIT TRAINING ===== */
const KNOWLEDGE_LEVEL_KEY = 'pcBuilder_knowledge_level_v1';
const KNOWLEDGE_TRAINING_DONE_KEY = 'pcBuilder_knowledge_training_done_v1';
const KNOWLEDGE_LIGHT_TRAINING_KEY = 'pcBuilder_knowledge_light_training_v1';
const KNOWLEDGE_TRAININGS = {
  1: {
    name: { ru: 'Новичок', en: 'Complete beginner' },
    intro: { ru: 'Начинаем буквально с нуля. Здесь не будет сложных слов: сначала поймём, что делает каждая основная деталь и как они работают вместе.', en: 'We start from absolute zero. There will be no wall of technical terms: first we will understand what the main parts do and how they work together.' },
    steps: [
      { icon: '🖥️', title: { ru: 'ПК — это команда деталей', en: 'A PC is a team of parts' }, text: { ru: 'Компьютер — это не одна волшебная коробка. Внутри работает команда. Процессор думает и раздаёт задачи, видеокарта рисует изображение, оперативная память держит то, что нужно прямо сейчас, накопитель хранит файлы, а материнская плата связывает всё это вместе.', en: 'A computer is not one magical box. It is a team. The CPU thinks and gives out tasks, the GPU draws the image, RAM holds what is needed right now, storage keeps files, and the motherboard connects everything together.' }, example: { ru: 'Представь маленькую команду: один человек решает, что делать, другой быстро рисует, третий держит всё нужное на столе, а шкаф хранит вещи на потом.', en: 'Think of a small team: one person decides what to do, another draws quickly, a third keeps current things on the desk, and a cabinet stores things for later.' } },
      { icon: '🧠🎨', title: { ru: 'Процессор говорит видеокарте, что рисовать', en: 'The CPU tells the GPU what to draw' }, text: { ru: 'Процессор, или CPU, выполняет команды и расчёты. Он может заниматься и графикой сам, но для постоянной тяжёлой отрисовки он не предназначен. Видеокарта, или GPU, специально сделана для того, чтобы очень быстро обрабатывать картинку. Поэтому даже очень сильный CPU не обязан быть быстрее старой видеокарты именно в рисовании.', en: 'The CPU, or processor, follows instructions and does calculations. It can work on graphics too, but it is not designed for heavy, constant graphics rendering. The GPU is specifically built to process images very quickly. That is why a very powerful CPU does not have to be faster than an old GPU at graphics work.' }, example: { ru: 'CPU говорит: «Вот что сейчас должно быть на экране». GPU берёт эту задачу и быстро рисует кадр.', en: 'The CPU says, “This is what should be on the screen.” The GPU takes that job and quickly draws the frame.' } },
      { icon: '⚪️⚪️', title: { ru: 'Пример с шариками', en: 'A simple ball example' }, text: { ru: 'Представь поле, по которому должны летать шарики. Хороший CPU знает, куда каждому шарику нужно двигаться, а хороший GPU быстро рисует их движения. Если GPU почти нет или он слишком слабый, шарики начинают телепортироваться: CPU знает, что должно происходить, но картинка не успевает нормально это показать. Если CPU слабый, он не успевает давать новые указания GPU. Если оба хорошие — шарики двигаются плавно и быстро.', en: 'Imagine a field where balls should fly around. A good CPU keeps track of where the balls should move, and a good GPU quickly draws that movement. With no useful GPU or a very weak one, the balls may look like they teleport: the CPU knows what should happen, but the image is not drawn fast enough. With a weak CPU, it cannot give the GPU new instructions fast enough. When both are good, the balls move smoothly and quickly.' }, exampleDemo: 'cpu-gpu-balls', example: { ru: 'CPU без нормальной GPU → «телепорт». Хороший CPU + хорошая GPU → плавное движение.', en: 'Weak or missing GPU → “teleporting” balls. Good CPU + good GPU → smooth, fast movement.' } },
      { icon: '🧩', title: { ru: 'RAM — это рабочий стол', en: 'RAM is your work desk' }, text: { ru: 'Оперативная память, или RAM, держит данные, которые нужны компьютеру прямо сейчас. Это можно представить как рабочий стол: чем он больше, тем больше вещей можно разложить одновременно. После выключения ПК содержимое RAM исчезает.', en: 'RAM is the memory used for things the computer needs right now. Think of it as a work desk: the bigger it is, the more things you can keep ready at once. RAM is cleared when the PC is turned off.' }, example: { ru: 'Открываешь игру, браузер и ещё несколько программ — всё это занимает место на рабочем столе RAM.', en: 'Open a game, a browser, and a few more programs: all of them take up space on the RAM desk.' } },
      { icon: '💾', title: { ru: 'Накопитель — это шкаф', en: 'Storage is the cabinet' }, text: { ru: 'SSD или HDD хранят Windows, игры, фотографии и другие файлы даже после выключения компьютера. Это уже не рабочий стол, а шкаф: вещи лежат там долго и не исчезают, когда ты выключаешь свет.', en: 'An SSD or HDD keeps Windows, games, photos, and other files even after the computer is turned off. It is more like a cabinet: the things stay there instead of disappearing when you switch the lights off.' }, example: { ru: 'RAM — вещи на столе прямо сейчас. SSD/HDD — вещи в шкафу, которые можно достать позже.', en: 'RAM is what is on the desk right now. SSD/HDD is what is stored in the cabinet for later.' } },
      { icon: '🔌❄️', title: { ru: 'Кто даёт питание и охлаждает всё это', en: 'Who powers and cools everything' }, text: { ru: 'Материнская плата соединяет детали. Блок питания, или PSU, даёт им электричество. Охлаждение отводит тепло. Если одна из этих вещей не подходит, мощные CPU и GPU сами по себе компьютер не спасут.', en: 'The motherboard connects the parts. The power supply, or PSU, provides electricity. Cooling removes heat. If one of these is unsuitable, powerful CPU and GPU parts cannot save the whole computer.' }, example: { ru: 'PSU — как электросеть для команды. Охлаждение — как вентилятор в жаркой комнате: оно не делает комнату умнее, но не даёт ей перегреться.', en: 'The PSU is like the electrical system for the team. Cooling is like a fan in a hot room: it does not make the room smarter, but it keeps heat under control.' } },
      { icon: '✅', title: { ru: 'Главное правило: детали должны подходить друг другу', en: 'The main rule: the parts must fit together' }, text: { ru: 'Нельзя просто взять самые дорогие детали и надеяться на чудо. CPU должен подходить к материнской плате, RAM — поддерживаться платой, GPU — получать нужное питание, а охлаждение — справляться с теплом. И только потом мы смотрим на общую мощность и цену.', en: 'You cannot just pick the most expensive parts and hope for magic. The CPU must fit the motherboard, the RAM must be supported by the board, the GPU must get enough power, and the cooling must handle the heat. Only then do we look at overall performance and price.' }, example: { ru: 'Запомни простую цепочку: «подходит → работает → не перегревается → сбалансировано».', en: 'Remember this simple chain: “fits → works → stays cool → is balanced.”' } }
    ]
  },
  2: {
    name: { ru: 'Знакомый с терминами', en: 'I have heard some terms' },
    intro: { ru: 'Названия вроде RTX, Ryzen и Intel уже знакомы. Теперь свяжем их с тем, что реально происходит в компьютере.', en: 'Names like RTX, Ryzen, and Intel already sound familiar. Now we connect those names to what actually happens inside the PC.' },
    steps: [
      { icon: '🏷️', title: { ru: 'RTX, Ryzen, Intel — что это вообще', en: 'RTX, Ryzen, Intel — what are they?' }, text: { ru: 'Это семейства и названия производителей. RTX — популярная линейка видеокарт NVIDIA, Ryzen — линейка процессоров AMD, Core — линейка процессоров Intel. Но одно слово не показывает всю мощность: важны конкретная модель и поколение.', en: 'These are product families and company names. RTX is a popular NVIDIA graphics-card family, Ryzen is an AMD CPU family, and Core is an Intel CPU family. A family name alone does not tell you the full performance level: the exact model and generation matter.' }, example: { ru: '«Ryzen» — это ещё не ответ на вопрос «насколько мощный процессор?». Нужно посмотреть конкретную модель.', en: '“Ryzen” is not enough to answer “how powerful is the CPU?” You still need the exact model.' } },
      { icon: '🎮', title: { ru: 'FPS и пинг отвечают за разное', en: 'FPS and ping are different things' }, text: { ru: 'FPS показывает, сколько кадров игра успевает отрисовать за секунду. Пинг показывает задержку связи с сервером. Можно иметь высокий FPS и плохой пинг одновременно — картинка будет плавной, но действия в сетевой игре будут приходить с задержкой.', en: 'FPS shows how many frames the game renders per second. Ping shows network delay to the server. You can have high FPS and bad ping at the same time: the picture can look smooth while online actions still arrive late.' }, example: { ru: 'FPS — насколько плавно бежит картинка. Пинг — насколько быстро сервер слышит тебя.', en: 'FPS is how smoothly the picture moves. Ping is how quickly the server hears you.' } },
      { icon: '🧠🎨', title: { ru: 'CPU и GPU работают в паре', en: 'CPU and GPU work as a pair' }, text: { ru: 'CPU не просто «сильный или слабый», и GPU тоже. Их задача — успевать работать вместе. Слабый CPU может не успевать давать GPU новые задачи, а слабый GPU может не успевать превращать эти задачи в готовые кадры.', en: 'A CPU and a GPU are not simply “strong” or “weak”. What matters is whether they can keep up with each other. A weak CPU may not feed the GPU with new work quickly enough, while a weak GPU may not turn that work into finished frames fast enough.' }, example: { ru: 'Это как кассир и работник кухни: один быстро принимает заказы, другой быстро готовит. Если один в десять раз медленнее, второй начинает ждать.', en: 'Think of a cashier and a kitchen worker: one takes orders quickly and the other cooks quickly. If one is much slower, the other starts waiting.' } },
      { icon: '🧠🎞️', title: { ru: 'RAM и VRAM — две разные памяти', en: 'RAM and VRAM are different memories' }, text: { ru: 'RAM — общая оперативная память ПК. VRAM — память видеокарты, где GPU держит графические данные. Больший объём помогает, но одна цифра сама по себе не превращает слабую деталь в мощную.', en: 'RAM is the PC’s general working memory. VRAM is memory on the graphics card where the GPU keeps graphics data. More capacity can help, but one number alone does not turn a weak part into a powerful one.' }, example: { ru: 'RAM — общий рабочий стол команды. VRAM — отдельный стол прямо у художника GPU.', en: 'RAM is the team’s general work desk. VRAM is the separate desk right next to the GPU artist.' } },
      { icon: '💾', title: { ru: 'HDD, SATA SSD и NVMe SSD', en: 'HDD, SATA SSD, and NVMe SSD' }, text: { ru: 'HDD — старый механический способ хранения. SATA SSD работает без движущихся частей и обычно намного быстрее HDD. NVMe SSD использует PCIe и обычно ещё быстрее. Главное: накопитель влияет на хранение и загрузки, а не заменяет CPU или GPU.', en: 'HDDs use a traditional mechanical design. SATA SSDs have no moving parts and are usually much faster. NVMe SSDs use PCIe and are usually faster again. The important point is that storage affects files and loading, not the job of the CPU or GPU.' }, example: { ru: 'Поменять HDD на SSD — это как заменить шкаф с ящиками на более быстрый шкаф. Это не превращает художника в супергероя.', en: 'Replacing an HDD with an SSD is like replacing a slow cabinet with a faster one. It does not turn the artist into a superhero.' } },
      { icon: '⚡', title: { ru: 'Ватты — это не «уровень качества»', en: 'Watts are not a quality score' }, text: { ru: 'Если PSU написан как 650 W, это говорит о доступной мощности. Но два блока по 650 W не обязаны быть одинаковыми по качеству. И наоборот, высокий ваттаж не означает, что системе столько реально нужно.', en: 'A 650 W PSU rating tells you about available power capacity. But two 650 W PSUs do not have to be equal in quality. A huge wattage number also does not mean the PC actually needs that much.' }, example: { ru: 'Это как бак бензина: большой бак даёт запас, но размер бака сам по себе не рассказывает, насколько хороша машина.', en: 'It is like a fuel tank: a larger tank gives more reserve, but tank size alone does not tell you how good the car is.' } },
      { icon: '🧩', title: { ru: 'Что проверять первым', en: 'What to check first' }, text: { ru: 'Сначала смотри на совместимость: CPU и сокет, RAM и поколение DDR, GPU и питание, размеры и разъёмы. После этого уже сравнивай производительность, цену и будущие апгрейды.', en: 'Start with compatibility: CPU and socket, RAM and DDR generation, GPU and power, physical size and connectors. After that, compare performance, price, and future upgrade room.' }, example: { ru: 'Сначала вопрос «запустится ли это вместе?», потом «насколько это быстро?».', en: 'Ask “will these parts work together?” before asking “how fast are they?”' } }
    ]
  },
  3: {
    name: { ru: 'Знаю базу', en: 'I know the basics' },
    intro: { ru: 'Теперь можно добавить немного настоящей логики сборки: как детали связываются между собой и почему две мощные детали всё равно могут дать плохую систему.', en: 'Now we can add real build logic: how parts connect and why two powerful components can still make a bad system.' },
    steps: [
      { icon: '🔩', title: { ru: 'Сокет — место для конкретного CPU', en: 'The socket is the CPU’s seat' }, text: { ru: 'Сокет — это физический разъём и часть платформы, в которую ставится процессор. Если CPU и материнская плата рассчитаны на разные платформы, они просто не смогут нормально работать вместе.', en: 'The socket is the physical CPU connection and part of the platform. If the CPU and motherboard are made for different platforms, they cannot properly work together.' }, example: { ru: 'Это как штекер и розетка разной формы: сколько бы ни стоил штекер, в неподходящую розетку он не влезет.', en: 'It is like a plug and a socket with different shapes: no matter how expensive the plug is, it still will not fit.' } },
      { icon: '🧩', title: { ru: 'DDR4 и DDR5 — не одно и то же', en: 'DDR4 and DDR5 are not the same' }, text: { ru: 'Поколение RAM должно поддерживаться материнской платой. Нельзя просто считать, что «оперативка есть — значит подойдёт». Кроме поколения, важны объём и количество доступных слотов.', en: 'The RAM generation must be supported by the motherboard. You cannot assume “it is RAM, so it will fit.” The generation, capacity, and available slots all matter.' }, example: { ru: 'У платы есть список того, какую «версию рабочего стола» она понимает. DDR4 и DDR5 — разные версии.', en: 'The motherboard supports specific “desk versions.” DDR4 and DDR5 are different versions.' } },
      { icon: '⚖️', title: { ru: 'Баланс CPU и GPU', en: 'CPU and GPU balance' }, text: { ru: 'Если CPU слишком слабый для GPU, он может не успевать давать ему новые задачи. Если слабый именно GPU, он просто не успевает рисовать то, что CPU ему говорит. Поэтому в сборке важна не только сила каждой детали, но и их сочетание.', en: 'If the CPU is too weak for the GPU, it may not feed it new work fast enough. If the GPU is the weak point, it simply cannot render what the CPU asks it to render quickly enough. A good build is about the combination, not only the strength of each part.' }, example: { ru: 'Слишком слабый CPU — художник ждёт указаний. Слишком слабый GPU — указания есть, но художник не успевает закончить рисунок.', en: 'Weak CPU — the artist waits for instructions. Weak GPU — the instructions are ready, but the artist cannot finish the drawing fast enough.' } },
      { icon: '🔌', title: { ru: 'БП должен подходить всей системе', en: 'The PSU must fit the whole system' }, text: { ru: 'Не считай БП только по одной цифре в ваттах. Важна рекомендованная мощность конкретной GPU, запас для всей системы и качество самого блока. Слишком слабый БП может стать реальной проблемой даже при отличных остальных деталях.', en: 'Do not judge a PSU by one wattage number. Look at the GPU recommendation, the whole system, available headroom, and the PSU itself. An undersized PSU can become a real problem even when the other parts are excellent.' }, example: { ru: 'Хорошая GPU без нормального питания — это как мощный двигатель без подходящей топливной системы.', en: 'A powerful GPU without proper power is like a powerful engine without a suitable fuel system.' } },
      { icon: '❄️', title: { ru: 'Охлаждение работает с теплом', en: 'Cooling deals with heat' }, text: { ru: 'Кулер не «добавляет FPS» напрямую. Его задача — уносить тепло от CPU и помогать компонентам работать стабильно. Сильный процессор с плохим охлаждением может не раскрыть свой потенциал.', en: 'A cooler does not directly “add FPS.” Its job is to move heat away from the CPU and help the parts stay stable. A powerful processor with poor cooling may not reach or keep its intended performance.' }, example: { ru: 'Это как вентилятор на жаре: он не делает тебя сильнее, но помогает тебе не перегреться.', en: 'It is like a fan on a hot day: it does not make you stronger, but it helps stop you from overheating.' } },
      { icon: '🏠', title: { ru: 'Размеры тоже являются совместимостью', en: 'Size is also compatibility' }, text: { ru: 'Даже если детали технически работают вместе, их нужно ещё физически разместить. Корпус должен вместить материнскую плату, GPU и охлаждение. Некоторые кулеры или видеокарты бывают слишком длинными или высокими.', en: 'Even when parts are technically compatible, they still have to physically fit. The case must have room for the motherboard, GPU, and cooling. Some coolers or graphics cards can simply be too tall or too long.' }, example: { ru: 'Это обычный чемодан: вещь может быть твоей и работать отлично, но если она не помещается внутрь, взять её с собой нельзя.', en: 'It is like a suitcase: the item can be yours and work perfectly, but if it does not fit inside, you still cannot take it.' } },
      { icon: '🧠', title: { ru: 'Теперь ты видишь ПК как систему', en: 'Now you can see the PC as a system' }, text: { ru: 'Хорошая сборка отвечает сразу на несколько вопросов: совместимы ли детали, хватает ли питания, справляется ли охлаждение, нет ли явного перекоса между CPU и GPU, и остаётся ли разумный запас на будущее.', en: 'A good build answers several questions at once: are the parts compatible, is there enough power, can cooling handle the heat, is there a clear CPU/GPU imbalance, and is there useful room for future upgrades?' }, example: { ru: 'Сборка — это не набор самых дорогих коробок. Это команда, где все должны успевать работать вместе.', en: 'A build is not a pile of expensive boxes. It is a team where everyone has to keep up.' } }
    ]
  },
  4: {
    name: { ru: 'Хорошо разбираюсь', en: 'I know my way around PCs' },
    intro: { ru: 'Здесь уже не учимся названиям. Учимся принимать решения: где сэкономить, где не экономить, как думать о платформе и почему «сильнее» не всегда означает «лучше».', en: 'This level is not about memorizing names. It is about making decisions: where to save, where not to save, how to think about a platform, and why “stronger” does not always mean “better.”' },
    steps: [
      { icon: '🧭', title: { ru: 'Смотри на платформу, а не только на бренд', en: 'Think about the platform, not only the brand' }, text: { ru: 'Сначала определяй платформу, сокет и поколение. Уже потом выбирай конкретный CPU или материнскую плату. Бренд важен, но совместимость начинается не с логотипа на коробке.', en: 'Start with the platform, socket, and generation. Only then compare the exact CPU or motherboard. The brand matters, but compatibility does not start with the logo on the box.' }, example: { ru: 'Два процессора одной марки могут относиться к разным платформам и требовать разные платы.', en: 'Two CPUs from the same brand can still belong to different platforms and require different motherboards.' } },
      { icon: '⚖️', title: { ru: 'Баланс — это про использование потенциала', en: 'Balance is about using the potential' }, text: { ru: 'Если поставить очень дорогую GPU рядом с намного более слабым CPU, часть возможностей GPU может не использоваться в некоторых задачах. И наоборот, огромный бюджетный CPU не спасёт слабую GPU в графически тяжёлой игре.', en: 'A very expensive GPU next to a much weaker CPU may leave some of the GPU’s potential unused in certain workloads. In the other direction, an expensive CPU cannot rescue a weak GPU in a graphics-heavy game.' }, example: { ru: 'Это как поставить гоночную машину в пробку: машина умеет ехать быстро, но условия не дают ей раскрыться.', en: 'It is like putting a race car in a traffic jam: the car can go fast, but the situation does not let it use that ability.' } },
      { icon: '💰', title: { ru: 'Бюджет нужно распределять', en: 'A budget should be distributed' }, text: { ru: 'Не существует смысла потратить почти всё на одну деталь и потом урезать всё остальное до минимума. Сначала определяешь задачу ПК, а затем распределяешь деньги туда, где они дадут реальную пользу.', en: 'There is little value in spending almost everything on one part and cutting the rest to the minimum. First define the job of the PC, then put money where it will actually help.' }, example: { ru: 'Игровому ПК может быть важнее вложиться в GPU, чем переплачивать за самый дорогой CPU, если остальная система при этом остаётся нормальной.', en: 'For a gaming PC, putting more money into the GPU can make more sense than buying the most expensive CPU, as long as the rest of the system stays solid.' } },
      { icon: '🔌', title: { ru: 'PSU: мощность, качество и запас', en: 'PSU: wattage, quality, and headroom' }, text: { ru: 'Ватты отвечают за доступную мощность. Качество — за то, насколько хорошо блок выполняет свою работу и защищает систему. Сертификат эффективности показывает эффективность, но не описывает весь блок целиком. И ещё нужен запас, а не работа БП постоянно на пределе.', en: 'Wattage is available power capacity. Quality is about how well the PSU does its job and protects the system. An efficiency certification describes efficiency, not the whole PSU. You also want headroom instead of running the unit at its limit all the time.' }, example: { ru: 'Большой бак полезен, но хороший автомобиль всё равно требует нормального двигателя и топливной системы.', en: 'A bigger tank can be useful, but a good car still needs a proper engine and fuel system.' } },
      { icon: '🌬️', title: { ru: 'Кулер и корпус — одна система', en: 'The cooler and case are one system' }, text: { ru: 'Можно поставить отличный кулер, но если горячий воздух не выходит из корпуса, проблема никуда не исчезнет. Смотри на высоту кулера, размеры GPU, вентиляторы и направление воздушного потока.', en: 'You can install an excellent cooler, but if hot air cannot leave the case, the problem does not disappear. Check cooler height, GPU size, fans, and airflow direction.' }, example: { ru: 'Хороший вентилятор внутри закрытой коробки не отменяет отсутствие выхода для горячего воздуха.', en: 'A good fan inside a closed box does not magically create a path for hot air to leave.' } },
      { icon: '🔮', title: { ru: 'Апгрейд нужно планировать заранее', en: 'Plan the upgrade path early' }, text: { ru: 'Проверь количество слотов RAM, поддерживаемый максимум, современные стандарты расширения, запас питания и место в корпусе. Иногда немного более дорогая платформа экономит деньги потом.', en: 'Check RAM slots, maximum supported capacity, expansion standards, power headroom, and physical room in the case. A slightly more capable platform can sometimes save money later.' }, example: { ru: 'Свободный слот RAM сейчас может означать намного более простой апгрейд через пару лет.', en: 'A free RAM slot today can make an upgrade much easier a couple of years later.' } },
      { icon: '🔍', title: { ru: 'Учись читать характеристики как доказательства', en: 'Read specifications as evidence' }, text: { ru: 'Не делай вывод только по рекламной надписи. Ищи сокет, тип RAM, размеры, разъёмы питания, лимиты платы, требования GPU и другие конкретные характеристики. Хорошая сборка строится на проверяемых фактах.', en: 'Do not make a decision from a marketing label alone. Look for the socket, RAM type, dimensions, power connectors, motherboard limits, GPU requirements, and other specific specifications. Good builds are based on checkable facts.' }, example: { ru: '«Должно подойти» — это предположение. «Вот спецификация, которая подтверждает, что подходит» — уже проверка.', en: '“It should fit” is a guess. “Here is the specification that proves it fits” is a check.' } }
    ]
  },
  5: {
    name: { ru: 'Готов собирать ПК', en: 'Ready to build a PC' },
    intro: { ru: 'Ты уже не нуждаешься в лекции по определениям. Этот курс — короткая проверка мышления перед реальной сборкой.', en: 'You do not need a lecture on basic definitions. This course is a practical check of the mindset you need before building a real PC.' },
    steps: [
      { icon: '📋', title: { ru: 'Начинай с задачи и бюджета', en: 'Start with the job and the budget' }, text: { ru: 'Сначала реши, для чего собирается ПК: игры, работа, учёба, монтаж или всё сразу. Затем задай бюджет. Только после этого выбирай детали. Иначе легко потратить деньги на компонент, который не является главным для твоих задач.', en: 'First decide what the PC is for: gaming, work, school, editing, or a mix. Then set the budget. Only after that choose the parts. Otherwise it is easy to spend money on a component that is not central to your workload.' }, example: { ru: 'Сначала вопрос «что я хочу получить от ПК?», а не «какой самый дорогой CPU купить?».', en: 'First ask “what do I want this PC to do?” rather than “what is the most expensive CPU I can buy?”' } },
      { icon: '🔩', title: { ru: 'CPU → сокет → материнская плата', en: 'CPU → socket → motherboard' }, text: { ru: 'Проверь платформу, сокет и поддержку конкретного CPU. Не полагайся только на одинаковый бренд. Смотри характеристики конкретной модели платы и процессора.', en: 'Check the platform, socket, and support for the exact CPU. Do not rely on brand matching. Check the specifications of the exact motherboard and processor models.' }, example: { ru: 'Если не можешь показать пальцем, где в спецификациях подтверждается совместимость, проверка ещё не закончена.', en: 'If you cannot point to the specification that proves compatibility, the check is not finished.' } },
      { icon: '🧩', title: { ru: 'RAM → поколение → объём → слоты', en: 'RAM → generation → capacity → slots' }, text: { ru: 'Проверь DDR-поколение, объём, частоты которые реально поддерживает платформа, и сколько слотов останется свободными. Не забывай, что заполненные слоты могут изменить удобство будущего апгрейда.', en: 'Check the DDR generation, capacity, supported memory speeds, and how many slots will remain free. Remember that filling all slots can affect how convenient future upgrades will be.' }, example: { ru: 'Если сейчас поставить максимум RAM во все слоты, будущий апгрейд может потребовать заменить старые модули.', en: 'If you fill every RAM slot now, a future upgrade may require replacing the existing modules.' } },
      { icon: '⚡', title: { ru: 'GPU → питание → запас', en: 'GPU → power → headroom' }, text: { ru: 'Проверь требования именно своей видеокарты, подходящие кабели и рекомендованную мощность БП. Не бери блок впритык и не думай, что огромная цифра ватт автоматически делает его хорошим.', en: 'Check the exact GPU requirements, the proper power connectors, and the recommended PSU capacity. Do not size the PSU with zero headroom, and do not assume a huge wattage number automatically means a great PSU.' }, example: { ru: 'Сначала GPU говорит, сколько питания ей нужно. Потом ты проверяешь, может ли PSU это дать всей системе с запасом.', en: 'The GPU tells you what power it needs. Then you check whether the PSU can provide that for the whole system with headroom.' } },
      { icon: '🧊', title: { ru: 'Охлаждение → корпус → airflow', en: 'Cooling → case → airflow' }, text: { ru: 'Проверь высоту CPU-кулера, длину GPU, поддержку радиаторов и вентиляторов корпусом. Потом подумай, куда попадает холодный воздух и куда выходит горячий.', en: 'Check CPU cooler height, GPU length, radiator support, and fan support in the case. Then think about where cool air enters and where hot air leaves.' }, example: { ru: 'Хорошая сборка должна не только влезть в корпус, но и иметь нормальный путь для воздуха.', en: 'A good build must not only fit inside the case; it also needs a sensible path for air.' } },
      { icon: '🛠️', title: { ru: 'В реальной сборке нельзя работать «на глаз»', en: 'Real assembly is not guesswork' }, text: { ru: 'ПК должен быть выключен и обесточен. Сверяйся с инструкциями конкретных компонентов, не дави на деталь через силу и не подключай кабели наугад. Если что-то не подходит — остановись и найди причину.', en: 'The PC should be powered off and disconnected. Follow the manuals for the exact components, never force a part into place, and do not guess cable connections. If something does not fit, stop and find out why.' }, example: { ru: '«Не вставляется» — это не команда давить сильнее. Это команда проверить сокет, ориентацию, крепление и совместимость.', en: '“It does not fit” is not a command to push harder. It is a command to check the socket, orientation, mounting, and compatibility.' } },
      { icon: '🏁', title: { ru: 'Финальная проверка перед покупкой', en: 'Final check before buying' }, text: { ru: 'Пройди цепочку: задача → бюджет → CPU/плата → RAM → GPU → PSU → охлаждение → корпус → накопитель → баланс. Если каждый пункт можешь объяснить, сборка уже не держится на удаче.', en: 'Run the chain: job → budget → CPU/motherboard → RAM → GPU → PSU → cooling → case → storage → balance. If you can explain every step, the build is no longer based on luck.' }, example: { ru: 'Твоя цель — не угадать правильную сборку, а уметь доказать, почему она правильная.', en: 'The goal is not to guess the right build. It is to explain why the build is right.' } }
    ]
  }
};
const KNOWLEDGE_LIGHT_COURSES = {
  1: {
    name: { ru: 'Первый ПК: с нуля', en: 'First PC: from zero' },
    intro: { ru: 'Практика для новичка. Мы уже разобрались, что делают детали — теперь посмотрим, как они ведут себя внутри настоящей сборки.', en: 'Practice for a beginner. We already covered what the parts do — now we will see how they behave inside a real build.' },
    lessons: [
      { trigger: 'enter', title: { ru: 'Начинаем практику', en: 'Practice starts here' }, text: { ru: 'Перед тобой уже готовая базовая сборка. Ничего страшного, если пока не всё запомнил: здесь можно спокойно нажимать, покупать и менять детали, а я буду объяснять, что именно произошло.', en: 'A basic build is already in front of you. It is fine if you do not remember everything yet: you can click, buy, and replace parts, and I will explain what just happened.' } },
      { trigger: 'catalog', title: { ru: 'Ищем детали', en: 'Finding parts' }, text: { ru: 'Открой каталог. Это место, где ты выбираешь конкретную деталь, а не просто абстрактное «CPU». Смотри на название и характеристики — потом мы научимся сравнивать их.', en: 'Open the catalog. This is where you choose an actual component, not just an abstract “CPU”. Look at the name and specs — we will learn to compare them next.' } },
      { trigger: 'purchase', title: { ru: 'Купил — ещё не установил', en: 'Bought does not mean installed' }, text: { ru: 'После покупки деталь просто попадает в инвентарь. Это как купить новую видеокарту в магазине и оставить коробку на столе: ПК пока продолжает работать со старой.', en: 'After buying, the component only goes into your inventory. It is like buying a new graphics card and leaving the box on the desk: the PC still uses the old one.' } },
      { trigger: 'install-cpu-gpu', title: { ru: 'CPU и GPU работают вместе', en: 'CPU and GPU work together' }, text: { ru: 'Теперь попробуй заменить CPU или GPU. CPU рассчитывает, что должно происходить, и даёт графические задачи. GPU быстро рисует результат. Если один из них сильно отстаёт, второй часть времени ждёт его.', en: 'Now try replacing the CPU or GPU. The CPU calculates what should happen and gives graphics tasks. The GPU draws the result quickly. If one falls far behind, the other spends part of its time waiting.' } },
      { trigger: 'install-ram-storage', title: { ru: 'RAM и накопитель — не одно и то же', en: 'RAM and storage are different' }, text: { ru: 'Потрогай RAM и накопитель через замену деталей. RAM нужна компьютеру прямо сейчас — как рабочий стол. SSD/HDD хранит данные надолго — как шкаф. Поэтому большой накопитель не заменяет большую RAM.', en: 'Try replacing the RAM and storage. RAM is what the computer needs right now — like a work desk. SSD/HDD keeps data for later — like a cabinet. A huge drive therefore does not replace having enough RAM.' } },
      { trigger: 'install-power-cooling', title: { ru: 'Электричество и тепло', en: 'Power and heat' }, text: { ru: 'PSU даёт компонентам электричество, а охлаждение отводит тепло. Сама по себе мощная деталь не полезна, если ей нечем нормально питаться или она перегревается.', en: 'The PSU provides electricity, while cooling removes heat. A powerful component is not useful by itself if it cannot be powered properly or becomes too hot.' } },
      { trigger: 'summary', title: { ru: 'Ты уже собираешь сам', en: 'You are already building' }, text: { ru: 'Ты прошёл путь от названий деталей до реальных замен. Теперь правило простое: сначала понять задачу детали, потом проверить, подходит ли она остальным, и только после этого гнаться за мощностью.', en: 'You moved from component names to real replacements. The rule is simple: understand what a part does, check that it fits the rest, and only then chase more performance.' } }
    ]
  },
  2: {
    name: { ru: 'Читаем железо', en: 'Reading hardware' },
    intro: { ru: 'Ты уже слышал названия вроде RTX, Ryzen и Intel. Теперь научимся смотреть дальше названия и понимать, что они реально означают в сборке.', en: 'You have heard names like RTX, Ryzen, and Intel. Now we will look past the names and understand what they actually mean in a build.' },
    lessons: [
      { trigger: 'enter', title: { ru: 'Название — это только начало', en: 'A name is only the beginning' }, text: { ru: 'RTX 5090, Ryzen 7, Core i5 — это названия семейств и моделей. Само по себе знакомое имя ещё не говорит, насколько деталь подходит именно этой сборке.', en: 'RTX 5090, Ryzen 7, and Core i5 are family and model names. A familiar name alone does not tell you whether a part fits this build.' } },
      { trigger: 'catalog', title: { ru: 'Смотрим характеристики', en: 'Read the specifications' }, text: { ru: 'Открой каталог и посмотри на разные модели одной категории. Сравнивай конкретные характеристики и производительность, а не только знакомую марку на коробке.', en: 'Open the catalog and compare different models in one category. Look at concrete specifications and performance, not just the familiar brand on the box.' } },
      { trigger: 'purchase', title: { ru: 'Покупаем с целью', en: 'Buy with a purpose' }, text: { ru: 'Теперь каждая покупка должна отвечать на вопрос: «Зачем мне это?» Более дорогая деталь имеет смысл, когда она реально помогает твоей задаче или даёт нужный запас на будущее.', en: 'Every purchase should answer one question: “Why do I need this?” A more expensive part makes sense when it actually helps your goal or gives useful headroom.' } },
      { trigger: 'install-cpu-gpu', title: { ru: 'Производительность — не одно число', en: 'Performance is not one number' }, text: { ru: 'CPU и GPU выполняют разные работы. В играх важны оба: CPU считает логику и задачи, GPU отвечает за большую часть отрисовки кадра. Поэтому одна огромная цифра у одной детали не гарантирует идеальный ПК.', en: 'CPU and GPU do different jobs. In games, both matter: the CPU handles logic and tasks, while the GPU does most of the frame rendering. One huge number on one component does not guarantee a perfect PC.' } },
      { trigger: 'install-ram-storage', title: { ru: 'Объём важнее бренда', en: 'Capacity matters more than a logo' }, text: { ru: 'Для RAM и накопителей сначала смотри на тип, объём и назначение. Бренд сам по себе не превращает 8 ГБ RAM в 32 ГБ и не делает маленький накопитель большим.', en: 'For RAM and storage, first look at type, capacity, and purpose. A brand name does not turn 8 GB of RAM into 32 GB or make a small drive larger.' } },
      { trigger: 'install-power-cooling', title: { ru: 'Сильнее — не значит автоматически лучше', en: 'More powerful is not automatically better' }, text: { ru: 'Очень мощный GPU требует подходящего питания и нормального охлаждения. Если остальные части не готовы, дорогой апгрейд может создать новую проблему вместо пользы.', en: 'A very powerful GPU needs suitable power and cooling. If the rest of the system is not ready, an expensive upgrade can create a new problem instead of helping.' } },
      { trigger: 'summary', title: { ru: 'Теперь ты читаешь названия правильно', en: 'Now you can read the names properly' }, text: { ru: 'Ты уже можешь смотреть на RTX, Ryzen и Intel не как на магические слова, а как на начало проверки конкретной модели, характеристик и совместимости.', en: 'You can now treat RTX, Ryzen, and Intel as the start of a check for a specific model, its specifications, and compatibility.' } }
    ]
  },
  3: {
    name: { ru: 'Баланс и совместимость', en: 'Balance and compatibility' },
    intro: { ru: 'Ты уже понимаешь базовые детали. Теперь Лайт-режим будет учить главному навыку сборщика: замечать конфликт или дисбаланс ещё до покупки.', en: 'You already know the basic parts. Now Light Mode will train the key builder skill: spotting conflicts and imbalance before buying.' },
    lessons: [
      { trigger: 'enter', title: { ru: 'Сильный ПК начинается с баланса', en: 'A strong PC starts with balance' }, text: { ru: 'Смысл хорошей сборки не в том, чтобы одна деталь была монстром. Важно, чтобы остальные могли нормально работать рядом с ней и не мешали ей раскрыться.', en: 'A good build is not about making one part a monster. The other parts need to work well beside it so they do not hold it back.' } },
      { trigger: 'catalog', title: { ru: 'Проверяем совместимость до покупки', en: 'Check compatibility before buying' }, text: { ru: 'Открой каталог и представь, что выбираешь следующую деталь. Смотри на сокет, тип RAM, питание, размеры и другие ограничения платформы ещё до покупки.', en: 'Open the catalog and pretend you are choosing the next part. Check socket, RAM type, power, dimensions, and platform limits before buying.' } },
      { trigger: 'purchase', title: { ru: 'Покупка — это решение', en: 'A purchase is a decision' }, text: { ru: 'Не смотри только на цену и мощность. Хорошая покупка одновременно отвечает задаче, подходит сборке и оставляет тебе нормальный путь для дальнейшего апгрейда.', en: 'Do not look only at price and performance. A good purchase matches the goal, fits the build, and leaves a sensible path for future upgrades.' } },
      { trigger: 'install-cpu-gpu', title: { ru: 'Баланс CPU и GPU', en: 'CPU and GPU balance' }, text: { ru: 'Если CPU слабее GPU, процессор может не успевать готовить новые задачи, и сильная видеокарта будет ждать. Если GPU слабее, CPU может быстро подготовить кадр, но видеокарте потребуется время, чтобы его нарисовать. Баланс зависит от самой игры и задачи.', en: 'If the CPU is much weaker than the GPU, it may not prepare new tasks fast enough and the GPU can sit waiting. If the GPU is weaker, the CPU may prepare the frame quickly while the GPU needs more time to render it. Balance depends on the game and workload.' } },
      { trigger: 'install-power-cooling', title: { ru: 'Проверяем PSU и охлаждение', en: 'Check the PSU and cooling' }, text: { ru: 'После мощного GPU смотри на БП и охлаждение. БП должен дать системе достаточно мощности с запасом, а охлаждение — убрать тепло от компонентов, которые его создают.', en: 'After adding a powerful GPU, check the PSU and cooling. The PSU needs enough capacity with headroom, while cooling must remove the heat produced by the components.' } },
      { trigger: 'install-ram-storage', title: { ru: 'Баланс есть и у памяти', en: 'Memory needs balance too' }, text: { ru: 'RAM должна быть достаточного объёма и подходящего поколения, а накопитель — соответствовать твоей задаче по скорости и объёму. Слишком мало одного ресурса тоже может мешать всей системе.', en: 'RAM needs enough capacity and the right generation, while storage should fit your needs for speed and capacity. Too little of one resource can still hold the whole system back.' } },
      { trigger: 'summary', title: { ru: 'Готово: ты проверяешь не одну деталь', en: 'Done: you check the whole system' }, text: { ru: 'Теперь твой взгляд должен идти по цепочке: задача → баланс → совместимость → питание → охлаждение → апгрейд. Именно так сборка перестаёт быть набором случайных дорогих деталей.', en: 'Your checklist is now: goal → balance → compatibility → power → cooling → upgrade path. That is how a build stops being a random pile of expensive parts.' } }
    ]
  }
};
let firstVisitTrainingActive = false;
let lightTrainingActive = false;
let knowledgeTrainingLevel = null;
let knowledgeTrainingStep = 0;

function knowledgeLocalized(value){
  if (value && typeof value === 'object' && value.ru !== undefined) return isEnglish() ? value.en : value.ru;
  return value ?? '';
}
function openKnowledgeLevelModal(){
  const modal=$('knowledgeLevelModal');
  if(!modal)return;
  closeFirstVisitPanel();
  modal.classList.remove('hidden');
  modal.setAttribute('aria-hidden','false');
}
function closeKnowledgeLevelModal(){
  const modal=$('knowledgeLevelModal');
  if(modal){modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');}
}
function openKnowledgeTraining(level){
  const data=KNOWLEDGE_TRAININGS[level];
  const modal=$('knowledgeTrainingModal');
  if(!data||!modal)return;
  knowledgeTrainingLevel=Number(level);
  knowledgeTrainingStep=0;
  localStorage.setItem(KNOWLEDGE_LEVEL_KEY,String(knowledgeTrainingLevel));
  closeKnowledgeLevelModal();
  modal.classList.remove('hidden');
  modal.setAttribute('aria-hidden','false');
  renderKnowledgeTrainingStep(true);
}
function renderKnowledgeTrainingStep(animate=false){
  const data=KNOWLEDGE_TRAININGS[knowledgeTrainingLevel];
  const step=data?.steps?.[knowledgeTrainingStep];
  if(!data||!step)return;
  const modal=$('knowledgeTrainingModal');
  const content=$('knowledgeTrainingContent');
  $('knowledgeTrainingBadge').textContent=`${isEnglish()?'Level':'Уровень'} ${knowledgeTrainingLevel} · ${knowledgeLocalized(data.name)}`;
  $('knowledgeTrainingStepLabel').textContent=`${knowledgeTrainingStep+1} / ${data.steps.length}`;
  $('knowledgeTrainingIcon').textContent=knowledgeLocalized(step.icon);
  $('knowledgeTrainingTitle').textContent=knowledgeLocalized(step.title);
  $('knowledgeTrainingText').textContent=knowledgeLocalized(step.text);
  const example=$('knowledgeTrainingExample');
  if(example){
    const ex=knowledgeLocalized(step.example);
    example.textContent=ex;
    example.classList.toggle('hidden',!ex);
  }
  const visual=$('knowledgeTrainingVisual');
  if(visual){
    visual.classList.toggle('hidden',step.exampleDemo!=='cpu-gpu-balls');
    visual.setAttribute('aria-label',isEnglish()?'CPU and GPU example':'Пример работы CPU и GPU');
  }
  const progress=100*(knowledgeTrainingStep+1)/data.steps.length;
  $('knowledgeTrainingProgressBar').style.setProperty('--progress',`${progress}%`);
  $('knowledgeTrainingPrevBtn').disabled=knowledgeTrainingStep===0;
  $('knowledgeTrainingNextBtn').textContent=knowledgeTrainingStep===data.steps.length-1?(isEnglish()?'Finish':'Завершить'):(isEnglish()?'Next':'Далее');
  const skipBtn=$('knowledgeTrainingSkipBtn'); if(skipBtn){ const lockSkip=Number(knowledgeTrainingLevel)>=1&&Number(knowledgeTrainingLevel)<=3; skipBtn.classList.toggle('hidden',lockSkip); skipBtn.disabled=lockSkip; }
  if(animate&&content){
    content.classList.remove('step-in');
    requestAnimationFrame(()=>content.classList.add('step-in'));
  }
}
...
function completeKnowledgeTraining(){
  ...
  closeKnowledgeTraining();
  localStorage.setItem(FIRST_VISIT_DONE_KEY,'1');
  localStorage.removeItem(FIRST_VISIT_TRAINING_KEY);
  firstVisitTrainingActive=false;
  if(Number(knowledgeTrainingLevel)>=1 && Number(knowledgeTrainingLevel)<=3){
    startKnowledgeLightCourse(Number(knowledgeTrainingLevel));
    return;
  }
  lightTrainingActive=false;
  showMainMenu();
}
function closeKnowledgeTraining(){
  const modal=$('knowledgeTrainingModal');
  if(modal){modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');}
}
function skipKnowledgeTraining(){
  const level=Number(knowledgeTrainingLevel);
  if(level>=1&&level<=3)return;
  localStorage.setItem(KNOWLEDGE_TRAINING_DONE_KEY,'0');
  localStorage.setItem(FIRST_VISIT_DONE_KEY,'1');
  localStorage.removeItem(FIRST_VISIT_TRAINING_KEY);
  firstVisitTrainingActive=false;
  lightTrainingActive=false;
...
function startFirstVisitOnboarding(){
  ...
  const legacyTraining=localStorage.getItem(FIRST_VISIT_TRAINING_KEY);
  if(legacyTraining){
    localStorage.removeItem(FIRST_VISIT_TRAINING_KEY);
  }
  showFirstVisitPanel(isEnglish()?'Hi. I am PC Builder.':'Привет. Я PC Builder.',isEnglish()?
    'This is a PC-building simulator with components, compatibility checks, a market, mini-games, and several difficulty levels. There is a lot going on here, so I can first figure out your computer knowledge and give you a suitable course.' :
    'Это симулятор сборки ПК с комплектующими, проверками совместимости, магазином, мини-играми и несколькими уровнями сложности. Здесь действительно много всего, поэтому сначала я могу определить твой уровень знаний и дать подходящее обучение.');
  const question=$('firstVisitQuestion'); if(question)question.textContent=isEnglish()?'This can get pretty complicated. Would you like a tutorial?':'Тут всё довольно сложно. Хочешь пройти обучение?';
}
function skipFirstVisitOnboarding(){
  localStorage.setItem(FIRST_VISIT_DONE_KEY,'1');
  localStorage.removeItem(FIRST_VISIT_TRAINING_KEY);
  firstVisitTrainingActive=false;
  lightTrainingActive=false;
...
function showFirstVisitPanel(title,text,badge='PC BUILDER · ПЕРВЫЙ ЗАХОД'){
  const panel=$('firstVisitIntro');if(!panel)return;
  const b=$('firstVisitBadge'),t=$('firstVisitTitle'),p=$('firstVisitText');
  if(b)b.textContent=isEnglish()?'PC BUILDER · FIRST VISIT':badge;
...
const FIRST_VISIT_DONE_KEY = 'pcBuilder_first_visit_18_done';
...
function renderLightModeUI(){
  ...
  const title=$('earnPanelTitle');
  if(title)title.textContent=active?(courseActive?`🎓 ${knowledgeLocalized(course.name)}`:(lightTrainingActive?'🎓 Обучение':'🎮 Заработок')):'🎮 Заработок';
  const lightTitle=$('lightModeTitle');
  if(lightTitle)lightTitle.textContent=courseActive?`🎓 ${knowledgeLocalized(course.name)}`:(isEnglish()?'Light Mode':'Лайт-режим');
  const lightIntro=$('lightModeIntroText');
  if(lightIntro)lightIntro.textContent=courseActive?knowledgeLocalized(course.intro):(isEnglish()?'This mode removes the extra economy and random events so you can calmly build and test a working PC.':'Здесь нет лишней экономики и случайных событий, поэтому можно спокойно собирать и проверять рабочий ПК.');
...
/* ===== v2.0.7 — GUIDED LIGHT MODE TRAINING ===== */
const KNOWLEDGE_LIGHT_GUIDED_COURSES = {
  1: {
    name: { ru: 'Первый ПК: знакомство и практика', en: 'First PC: guided practice' },
    intro: { ru: 'Сначала разберём твоё рабочее место. Потом ты сам сделаешь несколько действий, а я буду показывать, что именно произошло.', en: 'First we will learn your workspace. Then you will perform a few actions yourself, and I will show you what happened.' },
    tasks: [
      { action: 'inspect', value: 'pc', selector: '#pcWorkspaceLabel', title: {ru:'Задание: познакомься с «Мой ПК»',en:'Task: meet “My PC”'}, text: {ru:'Нажми на заголовок «Мой ПК». Это место показывает, какие детали сейчас стоят внутри компьютера. Здесь мы будем видеть результат своих решений.',en:'Click the “My PC” heading. This area shows which components are currently inside your computer. This is where we will see the results of our decisions.'}, resultTitle: {ru:'Вот твоё рабочее место',en:'This is your workspace'}, resultText: {ru:'Слева ты видишь сам компьютер и его детали, а внизу — общую мощность сборки. Мы будем менять детали и сразу смотреть, что изменилось.',en:'On the left you can see the computer and its components, with the total PC power below. We will change parts and immediately see what changes.'} },
      { action: 'inspect', value: 'earn', selector: '#earnWorkspaceLabel', title: {ru:'Задание: найди заработок',en:'Task: find your earnings'}, text: {ru:'Теперь нажми на «Заработок». Здесь находятся способы получить деньги в этом режиме. Деньги нужны для новых деталей.',en:'Now click “Earnings”. This is where you find ways to get money in this mode. Money is used for new components.'}, resultTitle: {ru:'Нашли второй главный экран',en:'Found the second main area'}, resultText: {ru:'«Мой ПК» показывает, что у тебя уже есть, а «Заработок» помогает получить ресурсы для следующего шага.',en:'“My PC” shows what you already have, while “Earnings” gives you resources for the next step.'} },
      { action: 'inspect', value: 'money', selector: '#lightMoneyWorkspaceLabel', title: {ru:'Задание: найди источник денег',en:'Task: find the money source'}, text: {ru:'Нажми на строку «Нужны деньги на детали?». Пока без покупки: просто познакомься с местом, где берутся деньги в Лайте.',en:'Click the “Need money for parts?” line. No purchase yet: just learn where money comes from in Light Mode.'}, resultTitle: {ru:'Деньги под контролем',en:'Money is under control'}, resultText: {ru:'В Лайте деньги даёт простой кликер. Чем сильнее компьютер, тем больше денег он будет давать за один клик.',en:'In Light Mode, money comes from a simple clicker. The stronger the PC, the more money each click will give.'} },
      { action: 'clicker', selector: '#lightClickerBtn', title: {ru:'Задание: заработай первые деньги',en:'Task: earn your first money'}, text: {ru:'Теперь нажми кнопку получения денег один раз. Это уже настоящее действие, которое меняет твой баланс.',en:'Now click the money button once. This is a real action that changes your balance.'}, resultTitle: {ru:'Первые деньги получены',en:'First money earned'}, resultText: {ru:'Готово. Теперь у тебя есть деньги на детали. Дальше познакомимся с магазином.',en:'Done. Now you have money for components. Next we will explore the shop.'} },
      { action: 'open-catalog', selector: '#openShopBtn', title: {ru:'Задание: открой каталог',en:'Task: open the catalog'}, text: {ru:'Нажми «Открыть каталог». Это твой магазин комплектующих. Здесь можно посмотреть детали, цены и характеристики.',en:'Click “Open catalog”. This is your component shop. Here you can see parts, prices, and specifications.'}, resultTitle: {ru:'Каталог открыт',en:'Catalog opened'}, resultText: {ru:'Теперь ты знаешь маршрут: рабочее место → заработок → каталог. Дальше начнём разбираться, что именно находится внутри ПК.',en:'Now you know the route: workspace → earnings → catalog. Next we will learn what is actually inside a PC.'} },
  ...
  2: {
    name: { ru: 'Читаем железо: практика', en: 'Reading hardware: guided practice' },
    intro: { ru: 'Ты уже слышал RTX, Ryzen и Intel. Теперь научимся читать не только названия деталей, но и сам интерфейс сборки.', en: 'You already know names like RTX, Ryzen, and Intel. Now we will learn to read the build interface and the hardware itself.' },
    tasks: [
      { action: 'inspect', value: 'pc', selector: '#pcWorkspaceLabel', title: {ru:'Задание: осмотри свой ПК',en:'Task: inspect your PC'}, text: {ru:'Нажми на «Мой ПК». Это твоя текущая сборка: здесь видно установленные детали и общую мощность.',en:'Click “My PC”. This is your current build: here you can see installed components and total power.'}, resultTitle: {ru:'Сборка перед глазами',en:'The build is in front of you'}, resultText: {ru:'Когда меняешь деталь, смотри сюда первым делом: установленное железо и его итоговая мощность находятся именно здесь.',en:'Whenever you change a component, look here first: the installed hardware and total power are shown here.'} },
      { action: 'inspect', value: 'earn', selector: '#earnWorkspaceLabel', title: {ru:'Задание: осмотри панель заработка',en:'Task: inspect the earnings panel'}, text: {ru:'Нажми на «Заработок». Здесь результат твоей сборки превращается в игровые ресурсы.',en:'Click “Earnings”. This is where your build result turns into game resources.'}, resultTitle: {ru:'Связь найдена',en:'The link is clear'}, resultText: {ru:'Чем лучше твоя сборка, тем сильнее меняется заработок. Поэтому апгрейд железа имеет смысл не только ради цифры мощности.',en:'A better build changes your earnings too. That is why hardware upgrades matter for more than just the power number.'} },
      { action: 'open-catalog', selector: '#openShopBtn', title: {ru:'Задание: открой каталог',en:'Task: open the catalog'}, text: {ru:'Открой каталог и посмотри, где находятся реальные модели видеокарт.',en:'Open the catalog and see where the real graphics-card models are.'}, resultTitle: {ru:'Каталог открыт',en:'Catalog opened'}, resultText: {ru:'Теперь можно читать не только бренд, но и серию и модель. RTX — это часть названия линейки, а не сама по себе гарантия совместимости.',en:'Now you can read not only the brand, but also the series and model. RTX is part of a product line name, not a guarantee of compatibility.'} },
      { action: 'select-category', value: 'GPU', selector: '.catalog-tab[data-cat="GPU"]', title: {ru:'Задание: открой GPU',en:'Task: open GPU'}, text: {ru:'Нажми GPU. Посмотри, как рядом существуют разные поколения и модели одной категории.',en:'Click GPU. Notice how different generations and models can exist in the same category.'}, resultTitle: {ru:'Модели имеют значение',en:'Models matter'}, resultText: {ru:'GTX 1050 и RTX 5090 — обе видеокарты, но это совсем разные уровни. Поэтому сравнивай конкретные модели, а не одно слово в названии.',en:'A GTX 1050 and an RTX 5090 are both graphics cards, but they are completely different performance classes. Compare specific models, not one word in the name.'} },
  ...
  3: {
    name: { ru: 'Совместимость: практика', en: 'Compatibility: guided practice' },
    intro: { ru: 'Здесь ты увидишь рабочее место, специально создашь ошибку и исправишь её сам.', en: 'Here you will explore the workspace, create a mistake on purpose, and fix it yourself.' },
    tasks: [
      { action: 'inspect', value: 'pc', selector: '#pcWorkspaceLabel', title: {ru:'Задание: проверь рабочее место',en:'Task: inspect the workspace'}, text: {ru:'Нажми «Мой ПК». Здесь будет видно, какую деталь ты поставил и какую мощность получил.',en:'Click “My PC”. This is where you will see what component you installed and how much power you got.'}, resultTitle: {ru:'Место для проверки найдено',en:'The check point is clear'}, resultText: {ru:'В нормальной сборке полезно постоянно возвращаться к этому блоку после покупки и установки.',en:'In a normal build, it is useful to return to this block after each purchase and installation.'} },
      { action: 'open-catalog', selector: '#openShopBtn', title: {ru:'Задание: открой каталог',en:'Task: open the catalog'}, text: {ru:'Открой каталог. Сейчас мы специально создадим несовместимость.',en:'Open the catalog. We are going to create an incompatibility on purpose.'}, resultTitle: {ru:'Начинаем эксперимент',en:'The experiment starts'}, resultText: {ru:'Наша цель не сломать ПК, а увидеть, как проверка ловит неправильную деталь.',en:'The goal is not to break the PC, but to see how the compatibility checker catches a wrong part.'} },
      { action: 'select-category', value: 'RAM', selector: '.catalog-tab[data-cat="RAM"]', title: {ru:'Задание: открой RAM',en:'Task: open RAM'}, text: {ru:'Нажми RAM. Для примера нам важно поколение памяти: DDR3, DDR4 или DDR5.',en:'Click RAM. For this example, the memory generation matters: DDR3, DDR4, or DDR5.'}, resultTitle: {ru:'Нашли важную характеристику',en:'Found an important specification'}, resultText: {ru:'Поколение RAM должно соответствовать тому, что поддерживает материнская плата. Название красивое, но совместимость важнее.',en:'The RAM generation must match what the motherboard supports. A nice name does not matter if the part is incompatible.'} },
      { action: 'purchase', itemId: 'ram-ddr3-4', selector: '[data-buy="ram-ddr3-4"]', title: {ru:'Задание: купи DDR3',en:'Task: buy DDR3'}, text: {ru:'Купи 4 GB DDR3. Именно эту память мы используем для эксперимента.',en:'Buy 4 GB DDR3. This is the memory we will use for the experiment.'}, resultTitle: {ru:'DDR3 куплена',en:'DDR3 purchased'}, resultText: {ru:'Она в инвентаре. Теперь специально установим деталь, которая не подходит этой платформе.',en:'It is in the inventory. Now we will intentionally install a part that does not fit this platform.'} },
      { action: 'install', itemId: 'ram-ddr3-4', selector: '[data-install="ram-ddr3-4"]', title: {ru:'Задание: установи DDR3',en:'Task: install DDR3'}, text: {ru:'Установи её. Ошибка здесь ожидаемая — нам важно увидеть её причину.',en:'Install it. The error is expected here — we want to see why it happens.'}, resultTitle: {ru:'Поймали несовместимость',en:'Compatibility issue caught'}, resultText: {ru:'Материнская плата ожидает DDR4, а мы поставили DDR3. Это главный принцип совместимости: деталь может быть рабочей, но не подходить именно этой системе.',en:'The motherboard expects DDR4, but we installed DDR3. That is the core compatibility rule: a working component can still be wrong for a specific system.'} },
...
function guidedLightActive() {
  const level = Number(state?.knowledgeLightTraining?.level);
  return isLightMode() && !!state?.knowledgeLightTraining?.active && !!guidedLightCourse(level) && level >= 1 && level <= 3;
}
function guidedLightLocalize(value) { return knowledgeLocalized(value); }
function guidedLightCurrentTask() {
  const st = state?.knowledgeLightTraining;
  const course = guidedLightCourse(st?.level);
  return course?.tasks?.[Number(st?.lesson) || 0] || null;
}
function guidedLightTarget(task = guidedLightCurrentTask()) {
  if (!task?.selector) return null;
  try { return document.querySelector(task.selector); } catch { return null; }
}
function clearGuidedLightTarget() {
  document.querySelectorAll('.light-guide-target').forEach((el) => el.classList.remove('light-guide-target'));
  const spotlight = $('lightGuideSpotlight');
  if (spotlight) { spotlight.classList.remove('show'); spotlight.style.removeProperty('top'); spotlight.style.removeProperty('left'); spotlight.style.removeProperty('width'); spotlight.style.removeProperty('height'); }
}
function positionGuidedLightTarget() {
  ...
  target.classList.add('light-guide-target');
  const rect = target.getBoundingClientRect();
  const pad = 7;
  spotlight.style.top = `${Math.max(4, rect.top - pad)}px`;
  spotlight.style.left = `${Math.max(4, rect.left - pad)}px`;
  spotlight.style.width = `${Math.max(10, rect.width + pad * 2)}px`;
  spotlight.style.height = `${Math.max(10, rect.height + pad * 2)}px`;
  spotlight.classList.add('show');
}
function scrollGuidedTargetIntoView() {
  const target = guidedLightTarget();
  if (!target) return;
  try { target.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' }); } catch { target.scrollIntoView(); }
  setTimeout(positionGuidedLightTarget, 220);
}
function setLightCoachText(kind, title, text) {
  const kindEl = $('lightCoachKind'), titleEl = $('lightCoachTitle'), textEl = $('lightCoachText');
  if (kindEl) kindEl.textContent = kind;
  if (titleEl) titleEl.textContent = title;
  if (textEl) textEl.textContent = text;
  const coach = $('lightCoach');
  if (coach) {
    coach.classList.add('guided');
    coach.classList.remove('show');
    requestAnimationFrame(() => coach.classList.add('show'));
  }
}
function guidedLightCloseOverlaysForTarget(target) {
  const shopModal = $('shopModal');
  if (shopModal && !shopModal.classList.contains('hidden') && !(target && shopModal.contains(target))) closeShop();
  const modalClosers = [
    ['osInstallModal', typeof cancelOsInstall === 'function' ? cancelOsInstall : null],
    ['osModal', typeof closeOsModal === 'function' ? closeOsModal : null],
    ['purchaseCouponModal', typeof closePurchaseCouponModal === 'function' ? closePurchaseCouponModal : null],
    ['extraCouponPurchaseModal', typeof closeExtraCouponPurchaseModal === 'function' ? closeExtraCouponPurchaseModal : null],
    ['couponReceivedModal', typeof closeCouponReceivedModal === 'function' ? closeCouponReceivedModal : null],
    ['compatibilityPurchaseModal', typeof closeCompatibilityPurchaseModal === 'function' ? closeCompatibilityPurchaseModal : null],
    ['hardcorePsuCertModal', typeof closeHardcorePsuCertModal === 'function' ? closeHardcorePsuCertModal : null],
    ['sellBlockedModal', typeof closeSellBlockedModal === 'function' ? closeSellBlockedModal : null],
    ['blackMarketPoliceWarningModal', typeof closeBlackMarketPoliceWarning === 'function' ? closeBlackMarketPoliceWarning : null],
    ['diagnosticModal', typeof closeDiagnostic === 'function' ? closeDiagnostic : null],
    ['saveModal', typeof closeSaveModal === 'function' ? closeSaveModal : null],
    ['resetConfirmModal', typeof closeResetConfirm === 'function' ? closeResetConfirm : null]
  ];
  modalClosers.forEach(([id, close]) => {
    const modal = $(id);
    if (modal && !modal.classList.contains('hidden') && close && !(target && modal.contains(target))) close();
  });
}

function showGuidedLightTask(animate = true) {
  if (!guidedLightActive()) return;
...
function showGuidedLightResult(task) {
  const course = guidedLightCourse(state.knowledgeLightTraining.level);
  setLightCoachText(isEnglish() ? '✅ Result' : '✅ Результат', guidedLightLocalize(task.resultTitle), guidedLightLocalize(task.resultText));
  clearGuidedLightTarget();
  const progress = $('lightTrainingProgress');
...
function guidedLightMatches(task, action, item, payload) {
  ...
  if (task.value !== undefined) {
    ...
    if (value !== task.value) return false;
  }
  if (task.itemId && task.itemId !== item?.id) return false;
  return true;
}
function advanceGuidedLightCourse(task) {
  if (!state?.knowledgeLightTraining) return;
  state.knowledgeLightTraining.lesson = Number(state.knowledgeLightTraining.lesson || 0) + 1;
  save();
  const course = guidedLightCourse(state.knowledgeLightTraining.level);
  if (state.knowledgeLightTraining.lesson >= (course?.tasks?.length || 0)) {
    clearTimeout(lightGuideTransitionTimer);
    lightGuideTransitionTimer = setTimeout(completeGuidedLightCourse, 1000);
    return;
  }
  clearTimeout(lightGuideTransitionTimer);
  lightGuideTransitionTimer = setTimeout(() => showGuidedLightTask(true), 1100);
}
function showKnowledgeLightLesson(action, item = null, payload = null) {
  if (!guidedLightActive()) return false;
  if (action === 'enter') {
...
function completeGuidedLightCourse() {
  if (!state?.knowledgeLightTraining) return;
  state.knowledgeLightTraining.completed = true;
  state.knowledgeLightTraining.active = false;
  state.lightTrainingEnabled = false;
...
function showKnowledgeLightWrongAction() {
  ...
  const task = guidedLightCurrentTask();
  if (!task) return;
  setLightCoachText(isEnglish() ? '🎯 Current task' : '🎯 Текущая задача', guidedLightLocalize(task.title), isEnglish() ? 'Use the highlighted element. We will move on automatically after the required action.' : 'Используй подсвеченный элемент. После нужного действия следующее задание откроется само.');
...
function lightGuideClickGuard(event) {
  ...
  if (raw instanceof Element && (target === raw || target.contains(raw))) {
    ...
    if (task?.action === 'inspect') {
      ...
      showKnowledgeLightLesson('inspect', null, task.value);
    }
    return;
  }
  if (raw instanceof Element && raw.closest('#lightCoach')) return;
  event.preventDefault();
  event.stopImmediatePropagation();
  showKnowledgeLightWrongAction();
}
function lightModeClick(){
  ...
  const reward=lightClickReward();
  state.money+=reward;
  lightClickerCount=Number(state.lightClickerCount)||0;
  lightClickerCount+=1;
  state.lightClickerCount=lightClickerCount;
  save();
  const count=$('lightClickerCount'); if(count)count.textContent=`${isEnglish()?'Clicks':'Кликов'}: ${lightClickerCount}`;
  const btn=$('lightClickerBtn'); if(btn)btn.textContent=`${isEnglish()?'Get':'Получить'} $${formatMoney(lightClickReward())}`;
  const money=$('money'); if(money)money.textContent=formatMoney(state.money);
  lightCoachAfterAction('clicker', null);
}
function lightCoachAfterAction(action,item){
  if(!isLightMode()||(!firstVisitTrainingActive&&!lightTrainingActive&&!knowledgeLightTrainingActive()))return;
  if(guidedLightActive()) {
    showKnowledgeLightLesson(action, item);
    return;
  }
  const issues=checkCompatibility();
  if(issues.length){const first=issues[0];let detail=first.detail;if(first.title.includes('CPU')&&first.title.includes('материн'))detail+=` Сокет — это физический разъём процессора: если платформы разные, такой CPU и плата вместе работать не смогут.`;else if(first.title.includes('RAM'))detail+=` У оперативной памяти тоже есть поколение, например DDR4 или DDR5. Оно должно соответствовать тому, что поддерживает материнская плата.`;else if(first.title.includes('БП'))detail+=` Блок питания должен иметь достаточную мощность для видеокарты и остальной системы; число ватт в названии БП — доступный запас питания.`;else if(first.title.includes('Охлаждение'))detail+=` Охлаждение должно успевать отводить тепло от процессора, иначе система не сможет работать в заданном режиме.`;lightCoachEvent('mistake',`Нашлась проблема: ${first.title}.`,`${detail} Это не штраф. Лайт-режим специально позволяет ошибаться без последствий, чтобы ты понял причинно-следственную связь. Исправь одну из конфликтующих деталей и снова проверь сборку.`,16000,true);}else{const power=currentPower();const actionText=action==='purchase'?`Ты добавил ${item?.name||'новую деталь'} в инвентарь. Покупка сама по себе ещё не меняет установленную сборку: после покупки деталь нужно установить.`:action==='install'?`Ты установил ${item?.name||'новую деталь'}. Теперь она участвует в расчёте мощности и проверках совместимости. ${lightComponentExplanation(item)}`:'Система проверена после твоего действия.';lightCoachEvent('praise',power>=250?'Сборка работает и уже достаточно мощная.':'Сборка работает.',`${actionText} Все основные компоненты сейчас совместимы. Текущая мощность — ${power}. Не гонись только за самым большим числом: дальше тебе нужно научиться смотреть на баланс, разумную стоимость и возможность будущего апгрейда.`,13000,true);}renderLightExamButton();
...
// The guided engine only owns the first three knowledge levels.
function positionLightGuideHandlers(){
  if (guidedLightActive()) positionGuidedLightTarget();
}
if (!document.documentElement.dataset.lightGuideGuardBound) {
  document.documentElement.dataset.lightGuideGuardBound = '1';
  document.addEventListener('click', lightGuideClickGuard, true);
  window.addEventListener('resize', positionLightGuideHandlers);
  window.addEventListener('scroll', positionLightGuideHandlers, true);
}
