...
$('tutorialSkipConfirmYes')?.addEventListener('click', skipTutorialToMainMenu);
$('tutorialSkipConfirmNo')?.addEventListener('click', closeTutorialSkipConfirm);
...
$('updateLogBtn').addEventListener('click', () => { renderUpdateLog(); openMenuModal('updateLogModal'); });
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape') {
    ...
    if (!$('tutorialSkipConfirmModal')?.classList.contains('hidden')) { closeTutorialSkipConfirm(); return; }
...
$('firstVisitLearnBtn')?.addEventListener('click', openKnowledgeLevelModal);
$('firstVisitSkipBtn')?.addEventListener('click', skipFirstVisitOnboarding);
document.querySelectorAll('[data-knowledge-level]').forEach((btn) => btn.addEventListener('click', () => openKnowledgeTraining(Number(btn.dataset.knowledgeLevel))));
$('knowledgeTrainingPrevBtn')?.addEventListener('click', () => { if (knowledgeTrainingStep > 0) { knowledgeTrainingStep -= 1; renderKnowledgeTrainingStep(true); } });
$('knowledgeTrainingNextBtn')?.addEventListener('click', () => { const total=KNOWLEDGE_TRAININGS[knowledgeTrainingLevel]?.steps?.length||0; if (knowledgeTrainingStep < total - 1) { knowledgeTrainingStep += 1; renderKnowledgeTrainingStep(true); } else completeKnowledgeTraining(); });
$('knowledgeTrainingSkipBtn')?.addEventListener('click', skipKnowledgeTraining);
