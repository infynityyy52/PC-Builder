...
addCatalog('Motherboard', 'Intel', 'B660 / B760', [
  ['mb-b660', 'B660 материнская плата', 420, 14], ['mb-b760', 'MSI PRO B760-P WIFI', 520, 17], ['mb-b760-ddr4', 'MSI PRO B760-P WIFI DDR4', 560, 19], ['mb-b760-wifi', 'B760 Wi-Fi Gaming', 690, 22]
], 0.66, [0.50, 2.35]);
...
function recommendationForIssue(issue) {
  let candidates = [];
  const installedIds = new Set(CATEGORIES.map((category) => state?.installed?.[category]).filter(Boolean));
...
const PROGRAMMING_TASKS = [
  ...
  { language:'JavaScript', title:'Проверка числа', difficulty:1, prompt:'Верни true, если число положительное.', template:'function isPositive(n) {\n  return ___;\n}', answer:'n > 0', hint:'Подсказка: сравни число с нулём.' },
  ...
  { language:'Python', title:'Квадрат числа', difficulty:2, prompt:'Верни квадрат x.', template:'def square(x):\n    return ___', answer:'x * x', hint:'Подсказка: умножь x само на себя.' },
  { language:'Python', title:'Приветствие', difficulty:2, prompt:'Верни строку Hello, и имя через пробел.', template:'def greet(name):\n    return ___', answer:'"Hello, " + name', hint:'Подсказка: сложи две строки.' },
  { language:'C++', title:'Максимум двух чисел', difficulty:3, prompt:'Верни большее из двух чисел.', template:'int max2(int a, int b) {\n    return ___;\n}', answer:'a > b ? a : b', hint:'Подсказка: используй тернарный оператор.' },
  { language:'C++', title:'Счётчик', difficulty:3, prompt:'Увеличь переменную count на 1.', template:'void increment(int& count) {\n    ___;\n}', answer:'count++;', hint:'Подсказка: оператор ++.' },
  { language:'Java', title:'Умножение', difficulty:3, prompt:'Верни произведение a и b.', template:'static int multiply(int a, int b) {\n    return ___;\n}', answer:'a * b', hint:'Подсказка: оператор умножения.' },
  { language:'Java', title:'Чётное число', difficulty:4, prompt:'Верни true, если n делится на 2 без остатка.', template:'static boolean isEven(int n) {\n    return ___;\n}', answer:'n % 2 == 0', hint:'Подсказка: оператор остатка %.' }
];
...
function programmingRequirementText() {
  ...
  if (programmingCanStart()) return `✅ Требования выполнены: ${cpu.name} · ${ram.name} (${kind})`;
  const problems=[];
...
function renderProgrammingPanel() {
  ...
  if(!programmingCanStart()) { area.innerHTML=`<div class="programming-card"><div class="programming-summary"><div><b>💻 Программирование</b><span>Пиши небольшие программы по готовому шаблону и получай деньги только после успешного завершения.</span></div><div class="programming-badge-row"><span class="badge">JavaScript · HTML · Python · C++ · Java</span></div></div><div class="programming-requirement locked">${programmingRequirementText()}</div></div>`; return; }
...
function checkProgrammingTask() {
  ...
  const reward=programmingReward(programmingTask); state.money+=reward; state.programmingStats=state.programmingStats||{completed:0}; state.programmingStats.completed+=1;
  ...
  programmingTask=null; save(); checkAchievements(); renderProgrammingPanel(); renderRewardInfo();
}
...
function renderShopItems(items, preserveOrder = false) {
  ...
  return `<div class="shop-items-grid">${sortedItems.map((item) => {
    ...
    const compatibilityStatus = itemCompatibilityStatus(item);
    ...
    const effectivePerf = effectiveItemPerformance(item);
    const storagePenalty = storagePerformancePenaltyPercent(item);
    ...
    return `<div class="shop-item ${unavailable && !bought ? 'sold-out' : ''} compatibility-item-${compatibilityStatus}"><div><div class="item-name">${item.name}</div><div class="item-desc">${item.desc}</div><div class="price-line"><span class="base-price">База: $${item.basePrice.toLocaleString('ru-RU')}</span><span class="price-change ${changeClass}">${changeText}</span>${performanceMarkup}${compatibilityMarkup}${manufacturerMarkup(item)}</div></div>${buttonHtml}</div>`;
  }).join('')}</div>`;
}
...
const UPDATE_LOG = `v1.15.6 — НАГРАДА ПРОГРАММИРОВАНИЯ И БОНУСЫ ПК
