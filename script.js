...
const MODE_CONFIG = {
  ...
  hard: {
    ...
    baseReward: 20,
    start: {
      CPU: 'starter-hard-cpu', RAM: 'starter-hard-ram', GPU: 'starter-hard-gpu', Storage: 'storage-hdd-120',
...
addCatalog('OS', 'Apple', 'macOS', [
  ['macos', 'macOS', 1820, 20, 'Операционная система Apple.', 'Apple']
], 0.42, [0.48, 2.60]);
...
function checkCompatibility() {
  ...
  if (!os) issues.push({ category: 'OS', title: 'Нет операционной системы', detail: 'Для запуска нужен установленный загрузочный образ ОС.', fixCategory: 'OS' });
...
function inventoryCategoryCount(category) {
  return SHOP.filter((item) => item.category === category && state?.owned?.[item.id]).length;
}
...
function start(mode) {
  ...
  Object.values(config.start).forEach((id) => { if (id) { state.owned[id] = true; state.starterProtected[id] = true; } });
  state.power = currentPower();
  closeGame();
  closeShop();
  save();
...
function sellItem(itemId) {
  ensureMarket();
  const item = itemById(itemId);
  if (!item || item.legacy || !state?.owned?.[itemId]) return false;
  normaliseStarterProtection();
  if (isStarterProtected(item)) {
...
function openExtraCouponPurchaseModal(itemId) {
  ensureMarket();
  const item = itemById(itemId);
  const marketItem = state.market.items[itemId];
...
function finishOsInstall() {
  ...
  const item = itemById(itemId);
  if (!item || item.category !== 'OS' || !state.owned[itemId] || isItemBroken(item)) return;
...
function openOsInstallModal(itemId) {
  const item = itemById(itemId);
  if (!item || item.category !== 'OS' || !state?.owned?.[itemId]) return;
  const modal = $('osInstallModal');
  if (!modal) return;
  cancelOsInstall();
...
function installItem(itemId) {
  ...
  if (item.category === 'OS') {
    if (state.installed.OS === item.id) return;
    stopAfk();
    openOsInstallModal(item.id);
    return;
  }
...
function render() {
  ...
  $('money').textContent = formatMoney(state.money); $('modeLabel').textContent = MODE_CONFIG[state.mode].label; renderPcCoupons();
...
function renderReplacement() {
  ...
  const body = owned.length
    ? `<div class="catalog-title">Купленные детали этого типа. Выбирай, что поставить.</div><div class="shop-items-grid">${owned.map((item) => {
      ...
      let buttonHtml;
      const starterBlocked = isStarterProtected(item);
      const sellButton = starterBlocked
        ? `<button class="small shop-buy danger" data-sell="${item.id}">Продажа запрещена</button>`
        : `<button class="small shop-buy danger" data-sell="${item.id}">Продать за $${formatMoney(salePriceFor(item.id))}</button>`;
      if (broken) {
        buttonHtml = `<div class="shop-actions"><button class="small shop-buy" data-repair="${item.id}">Починить за $${formatMoney(repairCost(item))}</button>${sellButton}</div>`;
      } else {
  ...
  if (mode === 'hardcore') {
    ...
    if (!group) {
      body = `<div class="catalog-title">${CATEGORY_LABELS[catalog.category]} → выбери производителя / тип</div><div class="catalog-tree">${groups.map((key) => {
        ...
        const countLabel = catalog.category === 'OS' ? `${count} ${count === 1 ? 'ОС' : 'ОС'}` : `${count} сер.`;
...
function bottleneckAchievementState() {
  ...
  for (const category of CATEGORIES) {
    ...
    const installedPerformance = Number(installed[category]?.performance) || 0;
...
function renderAchievements() {
  ...
  wrap.innerHTML = ACHIEVEMENTS.map((achievement) => {
    ...
    if (achievement.type === 'money') {
      ...
      body = `<div class="achievement-progress"><div class="achievement-progress-label">${result.ok ? '✅ Условие выполнено' : `Нужно: 8 мощных деталей + 1 хламовая${result.weakCategory ? ` • слабая: ${CATEGORY_LABELS[result.weakCategory]}` : ''}`}</div></div>`;
    ...
    } else if (achievement.type === 'super_buy') {
      body = `<div class="achievement-progress"><div class="achievement-progress-label">${unlocked ? '✅ Покупка двумя купонами выполнена' : 'Нужно купить любую деталь одновременно за EXTRA COUPON и обычный купон.'}</div></div>`;
    } else {
...
const UPDATE_LOG = `v1.14.2 — ХАРДКОР С WINDOWS XP
- Хардкор теперь начинается с Windows XP вместо Windows 7 Lite.
- Windows XP остаётся отдельной каталоговой ОС и используется как стартовая ОС хардкора.
...
- При наличии EXTRA COUPON товары SOLD OUT в каталоге получают кнопку «!EXTRA!».
- По «!EXTRA!» открывается модалка с текущей ценой детали и кнопками «Купить» / «Отмена».
...
- Поэтому засчитываются близкие к топу варианты вроде RTX 4090 вместо RTX 5090 или Windows 11 Home вместо Pro.

v1.13.2 — FIX HDD И HOLY BOTTLENECK
...
- Для обычного AFK и разгона нужна ОС не слабее Linux Mint (7 мощности).
- Для «Обучения ИИ» нужна ОС не слабее Windows 11 Home (16 мощности).
- «I need more powerful!», «Process or Factory», «Mining Farm» и «Terminator» дополнительно требуют Windows 11 Pro или macOS.
- Исправлено условие «Holy Bottleneck!»: ровно 8 топовых компонентов + 1 самый слабый каталоговый компонент, при этом сборка обязательно должна оставаться рабочей.

v1.12.0 — ОПЕРАЦИОННАЯ СИСТЕМА КАК КОМПОНЕНТ
- ОС стала полноценной деталью каталога и влияет на общую мощность ПК.
- Шанс события «Чёрная пятница» установлен на 5%.
...
- Стартовые ОС: Windows 10 Home — лёгкий режим, Windows 8.1 Home — сложный, Windows 7 Lite — хардкор.
- ОС можно покупать, устанавливать, продавать и учитывать в достижении полной коллекции.
- В обновлённом туториале добавлена подсказка про ОС.

v1.11.5 — 192 ГБ DDR5 И ОБНОВЛЁННЫЕ МАКСИМАЛЬНЫЕ ДОСТИЖЕНИЯ
- В каталог добавлена оперативная память 192 GB DDR5.
...
v1.11.4 — ИСПРАВЛЕН СБРОС ГЛОБАЛЬНЫХ ДОСТИЖЕНИЙ
- Исправлено восстановление сброшенной ачивки из старого сейва. Глобальное хранилище достижений теперь является единственным источником истины.

v1.11.3 — УВЕДОМЛЕНИЕ О ДОСТИЖЕНИИ ЗА КОЛЛЕКЦИЮ
...
- Ачивка I'll end this, for the order empire! теперь проверяется сразу после успешной покупки детали.
- Если после покупки собран весь актуальный каталог, достижение выдаётся немедленно, без необходимости сохраняться или открывать другие разделы.

v1.11.1 — ОЧИСТКА КАТАЛОГА ОТ УСТАРЕВШИХ ПОЗИЦИЙ
...
v1.11.0 — ПРОДАЖА ДЕТАЛЕЙ И НОВОЕ ДОСТИЖЕНИЕ
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape') { closeAllMenuModals(); closeDiagnostic(); closeShop(); pendingResetAfterSave = false; closeSaveModal(); closeResetConfirm(); closeHardcoreConfirm(); closePurchaseCouponModal(); closeCouponReceivedModal(); closeExtraCouponPurchaseModal(); closeSellBlockedModal(); }
});
